...
var addTab = function(id, title, url){
	tabpanel.addTab({
		id : id,  // id(不能重复)
		title : title, // 标题
		html : '<iframe width="100%" height="100%" src="${ctx}'+ url +'" frameborder="0"></iframe>' // 内容
	});
};
*/
...
TabPanel.prototype = {
  ...
  iterateFlush : function(iframeObj) {
    ...
    if(iframeObj!='undefined'&&iframeObj.window.frames.length>0)
    {
    ...
    {
      //将frame中的所有form提交
      if(iframeObj!='undefined'&&iframeObj.document.forms.length>0)
      {
