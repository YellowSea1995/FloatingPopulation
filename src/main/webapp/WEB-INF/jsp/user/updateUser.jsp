			...
			$(function(){			
	    		...
	    		success:function(msg){
                   ...
                   $.each(depts,function(){
                    	...
                    	$("<option>").attr("value",this.id).text(this.name).attr("selected",deptId==this.id).appendTo("#deptSelect");
                    })
                    
                    //将职位信息填充至下拉框
                    $.each(jobs,function(){
                    	//$("#jobSelect").append("<option value='"+this.id+"'>"+this.name+"</option>");
                    	$("<option>").attr("value",this.id).text(this.name).attr("selected",jobId==this.id).appendTo("#jobSelect");
                    })
...
<body>
	<table align="center">
		...
		<s:form action="/user/updateUser.jspx" method="post" id="updateUserForm" theme="simple">
			<!-- 防止表单重复提交  -->
            <s:token></s:token>
			...
			<tr>
				<td>
					登&nbsp;录&nbsp;名： ${user.userId }
					<s:textfield id="userId" type="hidden" name="user.userId" size="18"/>
				</td>
				<td>
					用户姓名：
